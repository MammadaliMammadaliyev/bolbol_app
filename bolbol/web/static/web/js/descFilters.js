...
let price_min = null;
let price_max = null;
let selected_colors = null;
let selected_fuel_type = null;
let body_type = null;
let engine_capacity_min = null;
let engine_capacity_max = null;
let year_min = null;
let year_max = null;
let transmission = null;
let city = null;
let mileage_min = null;
let mileage_max = null;
let drive_system = null;
...
document.querySelectorAll(".additional_chekings_item").forEach((item) => {
  item.addEventListener("click", (e) => {
    ...
    if (add_checks.includes(value)) {
      const index = add_checks.indexOf(value);
      add_checks.splice(index, 1);
    } else {
      add_checks.push(value);
    }
    console.log(add_checks);
  });
});
  ...
  if (type == "minLitr") {
    ...
    dropDownListItems.forEach(function (listItem) {
      listItem.addEventListener("click", function (e) {
        ...
        dropDownBtn.innerText = this.innerText;
        dropDownBtn.focus();
        engine_capacity_min = this.innerText;
        dropDownInput.value = this.dataset.value;
        dropDownList.classList.remove("dropdown__list--visible");
      });
    });
  ...
  if (type == "maxLitr") {
    ...
    dropDownListItems.forEach(function (listItem) {
      listItem.addEventListener("click", function (e) {
        ...
        dropDownBtn.innerText = this.innerText;
        dropDownBtn.focus();
        engine_capacity_max = this.innerText;
        dropDownInput.value = this.dataset.value;
        dropDownList.classList.remove("dropdown__list--visible");
      });
    });
...
const priceInputMin = document.querySelector("#price_input_min");
const priceInputMax = document.querySelector("#price_input_max");
const hpInputMin = document.querySelector("#hp_input_min");
const hpInputMax = document.querySelector("#hp_input_max");
const mileageInputMin = document.querySelector("#mileage_input_min");
const mileageInputMax = document.querySelector("#mileage_input_max");
const colorInput = document.querySelector("#color_input");
const for_credit = document.querySelector(".for_credit");
...
hpInputMin.addEventListener("keyup", (e) => {
  hp_min = e.target.value;
});
hpInputMax.addEventListener("keyup", (e) => {
  hp_max = e.target.value;
});
mileageInputMin.addEventListener("keyup", (e) => {
  mileage_min = e.target.value;
});
mileageInputMax.addEventListener("keyup", (e) => {
  mileage_max = e.target.value;
});

for_credit.querySelectorAll("input").forEach((input) => {
  input.addEventListener("click", (e) => {
    if (lease.includes(e.target.value)) {
      lease.splice(lease.indexOf(e.target.value), 1);
    } else {
      lease.push(e.target.value);
    }
  });
});
// const
const radioInputs = document.querySelectorAll(".for_car input");
const post = {};
...
radioInputs.forEach((input) => {
  input.addEventListener("click", (event) => {
    ...
    if (selectedValue === "all_car") {
      mileage_max = 0;
      mileage_min = 0;
    } else if (selectedValue === "new_car") {
      mileage_min = null;
      mileage_max = null;
    } else if (selectedValue === "old_car") {
      mileage_max = 0;
      mileage_min = 0;
    }
  });
});
const filterSubmit = document.querySelector(".filter_show_btn");
